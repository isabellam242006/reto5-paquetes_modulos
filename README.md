...
Primero creé el archivo "init" vacío y luego cree el archivo "figuras", que es donde están todas las funciones necesarias y los metí en la carpeta llamada Shape. Luego en el archivo "módulo" importé las funciones para poder utilizarlas, allí está el código principal:
...
class Isosceles(Triangle):
    def __init__(self):
        ...
        lengths = [edge.compute_length() for edge in self.get_edges()]
        self.is_isosceles = len(set(lengths)) == 2

class Equilateral(Triangle):
    def __init__(self):
        super().__init__(True)
        lengths = [edge.compute_length() for edge in self.get_edges()]
        self.is_equilateral = len(set(lengths)) == 1

class Scalene(Triangle):
    def __init__(self):
        super().__init__(False)
        lengths = [edge.compute_length() for edge in self.get_edges()]
        self.is_scalene = len(set(lengths)) == 3

class TriRectangle(Triangle):
    def __init__(self):
        super().__init__(False)
        angles = self.compute_inner_angles()
        self.is_right = 90 in angles
...
elif equilateral.is_equilateral:
    print("El triángulo es equilátero")
elif scalene.is_scalene:
    print("El triángulo es escaleno")
elif triRectangle.is_right:
    print("El triángulo es rectángulo")
else:
    print("El triángulo no es de un tipo específico")
...
```
Ahora los módulos importados como herencia de la clase Shape:
```css
shapes/
├── __init__.py
├── shaper.py
...
main.py
```
Shaper como la clase principal:
```python
import math
...
class Shape:
    ...
    def compute_perimeter(self):
        pass
```
La clase rectángulo que hereda de esta:
```python
from Shapes.shaper import *
...
class Square(Rectangle):
    def __init__(self):
        super().__init__(True)
```
La clase triángulo que también hereda de esta:
```python
from Shapes.shaper import *
...
class TriRectangle(Triangle):
    def __init__(self):
        ...
        self.is_right = 90 in angles
```
Y por último el código principal donde se importa todo:
```python
from Shapes import Triangle, Isosceles, Equilateral, Scalene, TriRectangle, Rectangle
